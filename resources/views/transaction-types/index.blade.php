    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <form action="{{ route('transaction-types.index') }}" method="GET" class="relative flex items-center">
                    ...
                    @if(request('search'))
                        <a href="{{ route('transaction-types.index') }}" class="ml-2 text-sm text-gray-500 hover:text-violet-500 underline whitespace-nowrap">
                            {{ __('Limpiar') }}
                        </a>
                    @endif
                </form>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xs rounded-xl border border-gray-200 dark:border-gray-700/60">
                    ...
                    <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                        @forelse ($transactionTypes as $transactionType)
                            <tr>
                                ...
                                <td class="px-5 py-3">
                                    <span class="text-gray-600 dark:text-gray-400">{{ $transactionType->description ?? '—' }}</span>
                                </td>
                                <td class="px-5 py-3 whitespace-nowrap text-right">
                                    <div class="flex justify-end gap-3">
                                        <a href="{{ route('transaction-types.show', $transactionType) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-5 py-10 text-center text-gray-500 dark:text-gray-400 text-sm">
                                    {{ __('No se encontraron tipos de transacción que coincidan con tu búsqueda.') }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            @if ($transactionTypes->hasPages() || $transactionTypes->total() > 0)
                <div class="px-5 py-4 border-t border-gray-100 dark:border-gray-700/60">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="text-sm text-gray-500 dark:text-gray-400 text-center sm:text-left">
                            {{ __('Mostrando') }}
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $transactionTypes->firstItem() ?? 0 }}</span>
                            {{ __('a') }}
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $transactionTypes->lastItem() ?? 0 }}</span>
                            {{ __('de') }}
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $transactionTypes->total() }}</span>
                            {{ __('resultados') }}
                        </div>
                        <div class="flex justify-center sm:justify-end">
                            {{ $transactionTypes->links('pagination::simple-tailwind') }}
                        </div>
                    </div>
                </div>
            @endif
