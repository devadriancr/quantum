<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        @php
            ...
            $partnerTypeMap = [
                'CUSTOMER' => ['label' => 'Cliente',   'class' => 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400'],
                'SUPPLIER' => ['label' => 'Proveedor', 'class' => 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400'],
                'BOTH'     => ['label' => 'Ambos',     'class' => 'bg-purple-100 text-purple-700 dark:bg-purple-500/20 dark:text-purple-400'],
            ];
        @endphp
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xs rounded-xl border border-gray-200 dark:border-gray-700/60 mb-6">
            ...
            <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex items-center justify-between gap-4 flex-wrap">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            {{ \App\Models\Container::TYPE_OPTIONS[$container->container_type] ?? $container->container_type }}
                        </p>
                    </div>
                </div>
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold {{ $statusColors[$container->status] ?? 'bg-gray-100 text-gray-600' }}">
            ...
            <div class="p-5">
                <ul class="divide-y divide-gray-100 dark:divide-gray-700/60 text-sm">
                    ...
                    <li class="grid grid-cols-1 md:grid-cols-2 py-3 gap-4">
                        <div class="flex items-center gap-3">
                            <span class="w-44 shrink-0 text-xs font-semibold uppercase text-gray-400 dark:text-gray-500">{{ __('Llegada real') }}</span>
                            <span class="font-medium text-gray-800 dark:text-gray-100">
                                {{ $container->actual_arrival_date
                                    ? \Carbon\Carbon::parse($container->actual_arrival_date)->format('d/m/Y')
                                    : '—' }}
                            </span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="w-44 shrink-0 text-xs font-semibold uppercase text-gray-400 dark:text-gray-500">{{ __('Hora real') }}</span>
                            <span class="font-medium text-gray-800 dark:text-gray-100">
                                {{ $container->actual_arrival_time
                                    ? \Carbon\Carbon::parse($container->actual_arrival_time)->format('H:i')
                                    : '—' }}
                            </span>
                        </div>
                    </li>
                    ...
                    <li class="grid grid-cols-1 md:grid-cols-2 py-3 gap-4 text-gray-600 dark:text-gray-400">
                        <div class="flex items-center gap-3">
                            ...
                            <span>{{ optional($container->created_at)->format('d/m/Y H:i') ?? '—' }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="w-44 shrink-0 text-xs font-semibold uppercase text-gray-400 dark:text-gray-500">{{ __('Actualizado') }}</span>
                            <span>{{ optional($container->updated_at)->format('d/m/Y H:i') ?? '—' }}</span>
                        </div>
                    </li>
                ...
                <header class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60 flex items-center justify-between gap-3 flex-wrap">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-gray-800 dark:text-gray-100">
                            {{ __('Documento') }}: {{ $document->document_number ?? '—' }}
                        </h3>
                    </div>
                    @php
                        $docStatusColors = [
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $docStatusColors[$document->document_status] ?? 'bg-gray-100 text-gray-600' }}">
                        {{ \App\Models\ShipmentDocument::STATUS_OPTIONS[$document->document_status] ?? $document->document_status }}
                    </span>
                </header>
        ...
        @if ($container->shippingDocuments->isEmpty())
