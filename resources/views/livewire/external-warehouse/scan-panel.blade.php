<div class="space-y-4">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700/60 p-5">
        ...
        <div class="relative">
            <input
                ...
                placeholder="Escanea aquí MY / MC / MH..."
            />
    ...
    @else
    <div class="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-xl p-4 flex items-center gap-3">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5 text-blue-500 shrink-0">
            <path d="M3.105 2.288a.75.75 0 0 0-.826.95l1.414 4.926A1.5 1.5 0 0 0 5.135 9.25h6.115a.75.75 0 0 1 0 1.5H5.135a1.5 1.5 0 0 0-1.442 1.086l-1.414 4.926a.75.75 0 0 0 .826.95 28.897 28.897 0 0 0 15.293-7.154.75.75 0 0 0 0-1.115A28.897 28.897 0 0 0 3.105 2.288Z"/>
        </svg>
        <span class="text-sm font-semibold text-blue-700 dark:text-blue-300">
            Viaje enviado — material en tránsito hacia {{ $locationTo }}
        </span>
    </div>
    @endif
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700/60 overflow-hidden">
        <header class="px-5 py-3 border-b border-gray-100 dark:border-gray-700/60 flex items-center justify-between">
            <h3 class="font-semibold text-gray-800 dark:text-gray-100 text-sm">Material en este viaje</h3>
            ...
            <table class="table-auto w-full dark:text-gray-300">
                <thead class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/20 border-b border-gray-100 dark:border-gray-700/60">
                    <tr>
                        ...
                        <th class="px-5 py-3 whitespace-nowrap"><div class="font-semibold text-center">{{ __('Ubicación') }}</div></th>
                        @if(!$isCompleted)
                    ...
                    @forelse($scans as $scan)
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900/10">
                            ...
                            <td class="px-5 py-3 whitespace-nowrap text-center">
                                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400">
                                    {{ $locationTo }}
                                </span>
                            </td>
                            @if(!$isCompleted)
                                <td class="px-5 py-3 text-center">
                                    <button
                                        type="button"
                                        class="remove-line-btn inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border-2 border-red-500 dark:border-red-500 bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors"
                                        data-line-id="{{ $scan->id }}"
                                        title="Remover del viaje"
                                    >
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $isCompleted ? 4 : 5 }}" class="px-5 py-10 text-center text-gray-400 dark:text-gray-500 text-sm italic">
                                Aún no se han escaneado piezas para este viaje.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
