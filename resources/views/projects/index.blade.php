            ...
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <form action="{{ route('projects.index') }}" method="GET" class="relative flex items-center">
                    ...
                    <input
                        ...
                        type="search"
                        placeholder="{{ __('Buscar por código, nombre o modelo...') }}"
                    ...
                    @if(request('search'))
                        <a href="{{ route('projects.index') }}" class="ml-2 text-sm text-gray-500 hover:text-violet-500 underline whitespace-nowrap">
                            {{ __('Limpiar') }}
                        </a>
                    @endif
                </form>
                ...
                <table class="table-auto w-full dark:text-gray-300">
                    <thead class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/20 border-t border-b border-gray-100 dark:border-gray-700/60">
                        <tr>
                            ...
                            <th class="px-5 py-3 whitespace-nowrap">
                                <div class="font-semibold text-left">{{ __('Modelo') }}</div>
                            </th>
                            <th class="px-5 py-3 whitespace-nowrap">
                    ...
                    <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                        @forelse ($projects as $project)
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-3 whitespace-nowrap">
                                    <span class="text-gray-600 dark:text-gray-400">{{ $project->model ?? '—' }}</span>
                                </td>
                                <td class="px-5 py-3 whitespace-nowrap">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-10 text-center text-gray-500 dark:text-gray-400 text-sm">
                                    {{ __('No se han encontrado proyectos que coincidan con tu búsqueda.') }}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <div class="px-5 py-4 border-t border-gray-100 dark:border-gray-700/60">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="text-sm text-gray-500 dark:text-gray-400 text-center sm:text-left">
                            {{ __('Mostrando') }}
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $projects->firstItem() ?? 0 }}</span>
                            {{ __('a') }}
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $projects->lastItem() ?? 0 }}</span>
                            {{ __('de') }}
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $projects->total() }}</span>
                            {{ __('resultados') }}
                        </div>
