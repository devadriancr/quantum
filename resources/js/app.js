...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Flatpickr — opciones base compartidas
  const fpBase = {
    mode: 'range',
    static: true,
  ...
  // Datepickers con rango por defecto (dashboards y gráficas)
  flatpickr('.datepicker:not([data-no-default])', {
    ...fpBase,
    defaultDate: [new Date().setDate(new Date().getDate() - 6), new Date()],
  });

  // Datepickers de filtro: sin rango por defecto, restaura el valor desde data-selected
  document.querySelectorAll('.datepicker[data-no-default]').forEach((input) => {
    const selectedRange = input.getAttribute('data-selected') || '';
    const opts = { ...fpBase };
    if (selectedRange && selectedRange.includes(' - ')) {
      const [from, to] = selectedRange.split(' - ');
      opts.defaultDate = [from.trim(), to.trim()];
    }
