    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">{{ __('Detalle de Especificación de Empaque') }}</h1>
            </div>
            <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                <a href="{{ route('packing-specifications.edit', $packingSpecification) }}"
                ...
                <ul class="divide-y divide-gray-100 dark:divide-gray-700/60 text-sm">
                    ...
                    <li class="grid grid-cols-1 md:grid-cols-2 py-3 gap-4">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $packingSpecification->name }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="w-44 shrink-0 text-xs font-semibold uppercase text-gray-400 dark:text-gray-500">{{ __('Cantidad') }}</span>
                            <span class="font-medium text-gray-800 dark:text-gray-100">
                                {{ $packingSpecification->quantity !== null ? number_format($packingSpecification->quantity) : '—' }}
                            </span>
                        </div>
                    </li>
                    ...
                    <li class="grid grid-cols-1 md:grid-cols-2 py-3 gap-4">
                        <div class="flex items-center gap-3">
                            ...
                            @if ($packingSpecification->status)
                    ...
                    <li class="grid grid-cols-1 md:grid-cols-2 py-3 gap-4 text-gray-600 dark:text-gray-400">
                        <div class="flex items-center gap-3">
                            ...
                            <span>{{ $packingSpecification->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="w-44 shrink-0 text-xs font-semibold uppercase text-gray-400 dark:text-gray-500">{{ __('Actualizado') }}</span>
                            <span>{{ $packingSpecification->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                    </li>
