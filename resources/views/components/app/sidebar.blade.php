<div class="min-w-fit">
    ...
    <div
        id="sidebar"
        class="flex lg:flex! flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-[100dvh] overflow-hidden w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:w-64! shrink-0 bg-white dark:bg-gray-800 p-4 transition-all duration-200 ease-in-out {{ $variant === 'v2' ? 'border-r border-gray-200 dark:border-gray-700/60' : 'rounded-r-2xl shadow-xs' }}"
    ...
    >
        ...
        <!-- Links (zona con scroll propio para que la flecha de colapsar quede fija abajo) -->
        <div class="grow min-h-0 overflow-y-auto no-scrollbar space-y-8">
            <!-- Pages group -->
            <div>
                    ...
                    href="#0" @click.prevent="open = !open; sidebarExpanded = true">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 @if(in_array(Request::segment(1), ['item-types', 'item-classes', 'measurement-units', 'warehouses', 'locations', 'partners', 'projects', 'project-prefixes', 'items'])){{ 'text-violet-500' }}@else{{ 'text-gray-400 dark:text-gray-500' }}@endif">
                            ...
                            <div class="flex shrink-0 ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                                <svg class="w-3 h-3 shrink-0 ml-1 fill-current text-gray-400 dark:text-gray-500 @if(in_array(Request::segment(1), ['item-types', 'item-classes', 'measurement-units', 'warehouses', 'locations', 'partners', 'projects', 'project-prefixes', 'items'])){{ 'rotate-180' }}@endif"
                    ...
                    <div class="lg:hidden lg:sidebar-expanded:block 2xl:block">
                        <ul class="pl-8 mt-1 @if(!in_array(Request::segment(1), ['item-types', 'item-classes', 'measurement-units', 'warehouses', 'locations', 'partners', 'projects', 'project-prefixes', 'items', 'packing-specifications', 'transaction-types', 'stock-limits', 'currencies', 'item-costs'])){{ 'hidden' }}@endif"
                            ...
                            <li class="mb-1 last:mb-0">
                                ...
                                href="{{ route('project-prefixes.index') }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4 shrink-0">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0Z" />
                                    </svg>
                                    <span class="text-sm font-medium lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                                        {{ __('Prefijos de Proyecto') }}
                                    </span>
                                </a>
                            </li>
                ...
                <li class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 bg-linear-to-r @if(in_array(Request::segment(1), ['roles', 'permissions', 'users'])){{ 'from-violet-500/[0.12] dark:from-violet-500/[0.24] to-violet-500/[0.04]' }}@endif"
                    x-data="{ open: {{ in_array(Request::segment(1), ['roles', 'permissions', 'users']) ? 1 : 0 }} }">

                    <a class="block text-gray-800 dark:text-gray-100 truncate transition @if(!in_array(Request::segment(1), ['roles', 'permissions', 'users'])){{ 'hover:text-gray-900 dark:hover:text-white' }}@endif"
                    href="#0" @click.prevent="open = !open; sidebarExpanded = true">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 @if(in_array(Request::segment(1), ['roles', 'permissions', 'users'])){{ 'text-violet-500' }}@else{{ 'text-gray-400 dark:text-gray-500' }}@endif">
                            ...
                            <div class="flex shrink-0 ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                                <svg class="w-3 h-3 shrink-0 ml-1 fill-current text-gray-400 dark:text-gray-500 @if(in_array(Request::segment(1), ['roles', 'permissions', 'users'])){{ 'rotate-180' }}@endif"
                                    :class="open ? 'rotate-180' : 'rotate-0'" viewBox="0 0 12 12">
                                    <path d="M5.9 11.4L.5 6l1.4-1.4 4 4 4-4L11.3 6z" />
                                </svg>
                            </div>
                    ...
                    <div class="lg:hidden lg:sidebar-expanded:block 2xl:block">
                        <ul class="pl-8 mt-1 @if(!in_array(Request::segment(1), ['roles', 'permissions', 'users'])){{ 'hidden' }}@endif"
                            :class="open ? 'block!' : 'hidden'">

                            <!-- Usuarios -->
                            @can('view users')
                            <li class="mb-1 last:mb-0">
                                <a class="flex items-center gap-2 text-gray-500/90 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition @if(Request::segment(1) === 'users'){{ 'text-violet-500!' }}@endif"
                            ...
                            @can('view roles')
                            <li class="mb-1 last:mb-0">
                                <a class="flex items-center gap-2 text-gray-500/90 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition @if(Request::segment(1) === 'roles'){{ 'text-violet-500!' }}@endif"
                            ...
                            <li class="mb-1 last:mb-0">
                                <a class="flex items-center gap-2 text-gray-500/90 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition @if(Request::segment(1) === 'permissions'){{ 'text-violet-500!' }}@endif"
        ...
        <div class="pt-3 hidden lg:inline-flex 2xl:hidden justify-end mt-auto shrink-0">
